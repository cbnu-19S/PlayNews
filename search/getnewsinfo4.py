...
def getNewsData(post, DataResult,inputkeyword):
    ...
    if date_mod is not None and writer is not None:
        DataResult.append({'cnt':cnt, 'title' : title, 'link' : link , 'date_first' : date_first.string, 'date_mod' : date_mod.string, 'newspaper':newspaper, 'writer':writer.string, 'context':context, 'img_link':img_link, 'img_alt':img_alt, 'keyword':keyword})
         ...
         DataResult.append({'cnt':cnt, 'title' : title, 'link' : link , 'date_first' : date_first.string, 'date_mod' : None, 'newspaper':newspaper, 'writer':writer.string, 'context':context, 'img_link':img_link, 'img_alt':img_alt, 'keyword':keyword})
    elif date_mod is not None and writer is None:
         DataResult.append({'cnt':cnt, 'title' : title, 'link' : link , 'date_first' : date_first.string, 'date_mod' : date_mod.string, 'newspaper':newspaper, 'writer':None, 'context':context, 'img_link':img_link, 'img_alt':img_alt, 'keyword':keyword})
    else:
        DataResult.append({'cnt':cnt, 'title' : title, 'link' : link , 'date_first' : date_first.string, 'date_mod' : None, 'newspaper':newspaper, 'writer':None, 'context':context, 'img_link':img_link, 'img_alt':img_alt, 'keyword':keyword})
    return

cursor = db.cursor(pymysql.cursors.DictCursor)
insert_sql = "INSERT INTO `newsinfo` (cnt,title,link,date_first,date_mod,newspaper,writer,context,img_link,img_alt,keyword) VALUES (%(cnt)s, %(title)s, %(link)s, %(date_first)s, %(date_mod)s, %(newspaper)s, %(writer)s, %(context)s, %(img_link)s, %(img_alt)s, %(keyword)s);"
# update_keyword_sql = "UPDATE newsinfo SET keyword=%s WHERE cnt BETWEEN 1 AND 10;"
#[CODE 0] - main 함수
def main():
    ...
    if ((jsonResult != None)):
        for post in jsonResult:
            if (cnt > 0):
                getNewsData(post, DataResult,keyword)
                cnt -= 1;
